...
def main():
    ...
    # ─── 4. Dashboard Report API ─────────────────────────────
    print(f"\n[4] ตรวจ Report API ({BACKEND_URL}/api/reports/today)")
    if backend_ok:
        try:
            r = requests.get(f"{BACKEND_URL}/api/reports/today", timeout=2)
            if r.status_code == 200:
                body = r.json()
                print("  ✅ Report API ใช้งานได้")
                print(f"     - attendance_total : {body.get('attendance_total', 0)}")
                print(f"     - drowsy_event_total : {body.get('drowsy_event_total', 0)}")
            else:
                print(f"  ⚠️  Report API ตอบกลับ HTTP {r.status_code}")
        except Exception:
            print("  ⚠️  Report API ยังไม่พร้อม — ตรวจว่า backend ใช้ไฟล์ dashboard_api.py")
    else:
        print("  ○ ข้ามการทดสอบ Report API เพราะ Backend ยังไม่ online")
 
    # ─── 5. สรุปคำแนะนำวันพรีเซนต์ ───────────────────────────
    print("\n" + "─"*60)
